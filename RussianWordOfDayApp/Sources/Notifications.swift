...
final class NotificationManager: NSObject, ObservableObject {
    ...
    /// Removes ALL pending requests (any identifier). Use only on a full
    /// "reset used words" — for settings-change rebuilds, prefer
    /// `removePending(withPrefix:)` so we don't nuke unrelated requests.
