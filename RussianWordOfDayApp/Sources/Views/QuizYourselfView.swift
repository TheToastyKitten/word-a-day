...
/// One question: Russian headword + four shuffled English choices (exactly one matches `word.english`).
private struct SelfQuizQuestion: Identifiable {
    ...
    var correctEnglish: String {
        word.english.trimmingCharacters(in: .whitespacesAndNewlines)
    }
}
...
struct QuizYourselfView: View {
    ...
    private func startQuiz() {
        ...
        let correctNorms = Set(pool.map { Self.normalizeEnglishKey($0.english) })
        ...
        for w in pool {
            let correct = w.english.trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            if wrong.count < 3 {
                for other in pool where other.id != w.id {
                    let cand = other.english.trimmingCharacters(in: .whitespacesAndNewlines)
